...
class MainProcess:
    ...
    def stitch(self):
        ...
        self.canvas:Image.Image = Image.new("RGB",(self.width+(len(image_files)-1)*self.gap,self.height),color=(255,255,255))
    ...
    def tovideo(self):
        ...
        # box = [0,0,int(self.height*self.proportion),self.height]
        # frames = []
        # for i in range(int(self.width/self.pixel_speed)):
        #     frames.append(self.canvas.crop(box))
        #     box[2] += self.pixel_speed
        #     box[0] += self.pixel_speed
        # frames.append(self.canvas.crop((self.width - 200,0,self.width,self.height)))
        ...
        video_clip0 = VideoClip(make_frame,duration=self.duration+1)
        # self.canvas.crop((self.width-self.height*self.proportion,0,self.width,self.height)).show()
        video_clip1 = ImageClip(numpy.array(self.canvas.crop((self.width-self.height*self.proportion,0,self.width,self.height))),duration=self.ending_time)
